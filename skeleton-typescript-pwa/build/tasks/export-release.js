...
gulp.task('export-sw', function(callback) {
  return runSequence(
    'bundle',
    'clean-export',
    'export-normalized-resources',
    'export-copy',
    'generate-export-service-worker',
    callback
  );
});

gulp.task('generate-export-service-worker', function(callback) {
  var path = require('path');
  var swPrecache = require('sw-precache');
  var rootDir = paths.exportSrv;

  swPrecache.write(path.join(rootDir, 'service-worker.js'), {
    ignoreUrlParametersMatching: [/^utm_/, /./],
    staticFileGlobs: [
      rootDir + '/**/*.{js,html,css,png,jpg,gif,svg,eot,ttf,woff}',
      rootDir + 'jspm_packages/npm/font-awesome@4.6.3/fonts/fontawesome-webfont.woff2',
    ],
    stripPrefix: rootDir
  }, callback);
});
